...
/*
void entropiaForDoc(FILE * fout, int doc, int chosendocs)
{
...
void entropiaForDocKnnMetrics(FILE * fout, int doc, int chosendocs)
{
	map <int, int> commonCategs;
	vector <InfoMetrics> docMetrics(NUM_OF_DOCS);
	for (int j = 0; j < NUM_OF_DOCS; ++j)
	{
		...
		docMetrics[j].metrics = docs[doc].getSimilarityKnn(docs[j]);  
		docMetrics[j].doc = j;
	}
...
void entropiaForTestDoc(FILE * fout, int doc, int chosendocs, ofstream & foutNumOfPairs)
{
	int totalNumOfPairsTest = 0;
	map <int, int> commonCategs;
	vector <InfoMetrics> docMetrics(NUM_OF_DOCS);
	for (int j = 0; j < NUM_OF_DOCS; ++j)
	{
		...
		docMetrics[j].metrics = testdocs[doc].getSimilarity(docs[j]);  
		docMetrics[j].doc = j;
	}
	sort(docMetrics.begin(), docMetrics.end(), compare_bigger);
	double totalDocs = 0;
...
#pragma omp critical
	...
	foutNumOfPairs << "num of pairs in test doc = " << totalNumOfPairsTest << endl;
}
...
void entropiaForDoc(FILE * fout, int doc, int chosendocs, vector <Document> & curDocs, int kindOfSimilarityFunction, bool trainFile)
{
	...
	for (auto it = commonCategs.begin(); it != commonCategs.end(); ++it)
	{
		// по-хорошему надо проверить формулу вычисления энтропии еще раз
		EntropiaPair temp;
		temp.doc = doc;
		temp.categ = it->first;
		double l = it->second;
...
int main()
{
	...
	// вывод информации об энтропии пары документ-категория в файл
	int chosendocs = 30;  // было для 30
	/*FILE * fout;
